...
describe("SpanishFood Integration Test", () => {
  ...
  test("adds items to cart and calculates totals", () => {
    ...
    const finalFoodItem = within(expectedFoodItem).getByText(
      firstFoodItem.name
    );
    ...
    const cartItemRow = within(insideTheCart)
      .getByText(firstFoodItem.name)
      .closest("li");
    ...
    const finalShipping = within(insideTheCart).getByText(
      `€${expectedShipping}`
    );
    expect(finalShipping).toBeInTheDocument();

    const expectedTotal = (
      Number(expectedSubtotal) +
      Number(expectedTax) +
      Number(expectedShipping)
    ).toFixed(2);
    const finalTotal = within(insideTheCart).getByText(`€${expectedTotal}`);
    expect(finalTotal).toBeInTheDocument();

    const checkoutButton = within(insideTheCart).getByRole("button", {
      name: /cash out/i,
    });
